...
class TrainerSingle:
    ...
    def _run_epoch(self, epoch: int):
        ...
        print(f"{'-' * 90}\n[GPU{self.gpu_id}] Epoch {epoch:.2d} | Batchsize: {self.const['batch_size']} | "
              f"Steps: {len(self.trainloader)} | LR: {self.optimizer.param_groups[0]['lr']:.4f} | "
              f"Loss: {loss / len(self.trainloader)} | Acc: {100 * self.train_acc.compute().item():.2f}%",
              flush=True)
    ...
    def test(self, final_model_path: str):
        ...
        print(f"[GPU{self.gpu_id}] Test Acc: {100 * self.valid_acc.compute().item():.4f}%",
              flush=True)
...
def main_single(gpu_id: int, final_model_path: str):
    ...
    train_dataloader, test_dataloader = cifar_dataloader_single(train_dataset, test_dataset, const['batch_size'])
    model = cifar_model()
...
if __name__ == '__main__':
    gpu_id = 0
    final_model_path = Path("./trained_models/CIFAR10/CIFAR10_single_epoch14.pt")
